...
const Main = (props) => {
  ...
  const categories = ["all", ...new Set(meals.map((meal)=>{return(meal.category)}))] ;
  // search
  const search = (word)=>{
    if(word !== ""){
      const searchedMeals = meals.filter((meal)=> meal.title === word)
  ...
  return (
    <main className='pd-y f-wth'>
      ...
      <SectionHeader categories = {categories} categoryFilterFunction = {categoryFilter} />
